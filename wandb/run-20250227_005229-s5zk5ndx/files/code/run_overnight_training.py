...
def monitor_process(process, wandb_run=None):
    ...
    """
    with Status("[bold green]Training in progress...", console=console) as status:
        ...
        while process.poll() is None:
            ...
            if current_time - last_update > 30:
                ...
                status_msg = f"[bold green]Training in progress... Elapsed: {int(hours)}h {int(minutes)}m {int(seconds)}s"
...
def main():
    ...
    with console.status("[bold green]Preparing overnight training...") as status:
